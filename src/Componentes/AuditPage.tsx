...
// =================================================================
interface FilterState { actor: string; action: string; entity: string; q: string; from?: string; to?: string; }
interface AuditEntry { id: string; actor: string; action: string; entity: string; entityId: string | null; before: string | null; after: string | null; comment: string | null; timestamp: string; }


// =================================================================
// 2. COMPONENTE AuditFilters (Consolidado)
// =================================================================
function AuditFilters({ value, onChange, actors, actions, entities, onReset }: { value: FilterState, onChange: (filters: FilterState) => void, actors: string[], actions: string[], entities: string[], onReset: () => void }) {
    ...
    return (
        <div className="card shadow-sm mb-3">
            ...
            <div className="card-body">
                <div className="mb-3">
                    <label className="form-label small text-muted">Busca Livre (ID/Comentário)</label>
                    <input 
                        type="text" 
...
// =================================================================
function AuditTable({ entries, onExport }: { entries: AuditEntry[], onExport: (rows: AuditEntry[]) => void }) {
    ...
    // Modal de Detalhes
    const DetailModal = ({ entry, onClose }: { entry: AuditEntry, onClose: () => void }) => {
        return (
            <>
                    ...
                    <div className="modal-dialog modal-lg modal-dialog-centered" role="document">
                        <div className="modal-content">
                            ...
                            <div className="modal-body">
                                <p><strong>Ação:</strong> <span className={`badge ${entry.action === 'create' ? 'bg-success' : entry.action === 'delete' ? 'bg-danger' : 'bg-warning text-dark'}`}>{entry.action.toUpperCase()}</span></p>
                                <p><strong>Entidade:</strong> {entry.entity} (ID: {entry.entityId || '-'})</p>
                                <p><strong>Ator:</strong> {entry.actor}</p>
                                <p><strong>Data/Hora:</strong> {formatDate(entry.timestamp)}</p>
                                <p><strong>Comentário:</strong> {entry.comment || '-'}</p>
                                <hr />
                                <div className="row">
                                    <div className="col-6">
                                        <h6>Antes (Before)</h6>
                                        <pre className="p-2 border bg-light small overflow-auto" style={{ maxHeight: '300px', whiteSpace: 'pre-wrap' }}>{entry.before || 'N/A'}</pre>
                                    </div>
                                    <div className="col-6">
                                        <h6>Depois (After)</h6>
                                        <pre className="p-2 border bg-light small overflow-auto" style={{ maxHeight: '300px', whiteSpace: 'pre-wrap' }}>{entry.after || 'N/A'}</pre>
                                    </div>
                                </div>
                            </div>
    ...
    return (
        <div className="card shadow-sm">
            <div className="card-header bg-white d-flex justify-content-between align-items-center">
                ...
                <button className="btn btn-sm btn-outline-primary" onClick={() => onExport(entries)}>
                        ...
                        {entries.length > 0 ? (
                            entries.map((e) => (
                                <tr key={e.id}>
                                    <td>{e.id}</td>
                                    <td>{e.actor}</td>
                                    <td><span className={`badge ${e.action === 'create' ? 'bg-success' : e.action === 'delete' ? 'bg-danger' : 'bg-warning text-dark'}`}>{e.action.toUpperCase()}</span></td>
                                    <td>{e.entity}</td>
                                    <td>{formatDate(e.timestamp)}</td>
                                    <td className="text-truncate" style={{ maxWidth: 200 }}>{e.comment}</td>
                                    <td>
                                        <button 
...
const MOCK: AuditEntry[] = [
  {
    ...
    entityId: "u-123",
    before: null,
    after: JSON.stringify({ name: "João", email: "joao@ex.com", role: "Solicitor" }, null, 2),
...
export default function AuditPage() {
  ...
  const [filters, setFilters] = useState<FilterState>({ actor: "", action: "", entity: "", q: "", from: undefined, to: undefined });
  ...
  const filtered = useMemo(() => {
    return entries.filter((e) => {
      ...
      if (filters.q) {
        ...
        const hay = `${e.id} ${e.actor} ${e.action} ${e.entity} ${e.entityId ?? ""} ${e.comment ?? ""} ${e.before ?? ""} ${e.after ?? ""}`.toLowerCase();
      ...
      <div className="row">
        ...
        <aside className="col-12 col-md-4 col-lg-3 mb-3">
          <AuditFilters
            ...
            actions={actions}
            entities={entities}
            onReset={() => setFilters({ actor: "", action: "", entity: "", q: "", from: undefined, to: undefined })}
          />
          ...
          <div className="card mt-3">
            <div className="card-body">
              <h6>Ações</h6>
              <p className="small text-muted">Exporte o conjunto de dados filtrado.</p>
              <button className="btn btn-outline-primary w-100" onClick={() => handleExport(filtered)}>
